...
sd_imgnet = [0.229, 0.224, 0.225]

INV_mean_imgnet = [-m for m in mean_imgnet]
INV_sd_imgnet = [1.0 / s for s in sd_imgnet]
...
def imagenet_preprocess():
  return T.Normalize(mean=mean_imgnet, std=sd_imgnet)



class VgSceneGraphDataset(Dataset):
  def __init__(self, vocab, path_file_h5, image_dir, image_size=(256, 256),
    ...
    self.data = {}
    with h5py.File(path_file_h5, 'r') as f:
      for k, v in f.items():
        if k == 'image_paths':
          self.image_paths = [i.decode() for i in list(v)] 
        else:
